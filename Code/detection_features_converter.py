...
if __name__ == '__main__':
    ...
    if os.path.exists(train_ids_file) and os.path.exists(val_ids_file):
        with open(train_ids_file, 'r') as f:
            reader = csv.reader(f)
            train_imgids = list(reader)
        for i in range(len(train_imgids)):
            train_imgids[i]=int(train_imgids[i][0])
        with open(val_ids_file, 'r') as f:
            reader = csv.reader(f)
            val_imgids = list(reader)
        for i in range(len(val_imgids )):
            val_imgids [i]=int(val_imgids [i][0])
    else:
